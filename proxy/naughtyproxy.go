/*
Package proxy provide NaughtyProxy as a HTTP reverse proxy that can
connects to both http/https backend server.
It uses hooked print function to print http(s) request/response.
It also provides a call back function that provide a method modifying
...
// NaughtyProxy is a reverse proxy server that acting as
// man-in-the-middle and printing http request/response for
// debugging purposes
...
func (p *NaughtyProxy) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	...
	*outreq = *req // includes shallow copies of maps, but okay
	...
	outreq.ProtoMinor = 1
	outreq.Close = false
	// fix host part to avoid some server reject request
	if p.Backend != nil {
	...
	if p.Callback != nil {
		p.Callback(req, outreq)
	}
	p.Speaker.SpeakRequest(outreq)
...
func (p *NaughtyProxy) copyResponse(dst io.Writer, src io.Reader) {
